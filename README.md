...
Firebase was written for python 3.6+ and will not work correctly with lower version of python.
...
```firebaseApp.auth()``` - [Authentication](#authentication)

```firebaseApp.database()``` - [Database](#database)

```firebaseApp.storage()``` - [Storage](#storage)
